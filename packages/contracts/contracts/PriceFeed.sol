...
* Chainlink oracle.
*用于主网部署的PriceFeed，将连接到Chainlink的live ETH:USD聚合器参考
*合同，和包装合同TellorCaller，它连接到TellorMaster合同。
＊
* PriceFeed使用Chainlink作为主oracle, Tellor作为备用oracle。它包含以下逻辑
*基于oracle故障、超时和返回主节点的条件切换oracle
* Chainlink甲骨文。
*/
